...
output=/home/mbxsh12/ind_proj/minimaptest #set to wherever you want all your files
...
mkdir -p barcodes "$train" "$splitfiles"  "$bamfiles1" "$sortbamdir" "$coveragedir" "$bigwigfolder" "$graphoutput" "$pyexpdir" "$headerbamdir" "$sortbamdir2" "$csvoutput" tmp "$aligndir"

removedirs=(barcodes "$train" "$splitfiles" "$bamfiles1" "$sortbamdir" "$ref_fai" "$Crossover_csv_script" tmp "$sortbamdir2" "$headerbamdir" "$pyexpdir" "$aligndir")
...
for barcode in barcodes/*; do
	...
	echo "Sorting $filename and removing secondary reads"
	#samtools sort "$alignoutput" -o "$samtoolssort_output"
	samtools sort "$alignoutput" | samtools view -b -F 256 -o "$samtoolssort_output"
...
for bamfile in "$sortbamdir"/*.bam; do
	...
	samtools view -bt  "$ref_fai" -o "$bamoutput" "$bamfile"
